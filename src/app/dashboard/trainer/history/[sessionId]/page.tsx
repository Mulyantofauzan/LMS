...
export default async function TrainerHistoryDetailPage({
  params,
}: {
  ...
  .where(and(eq(trainingSessions.id, sessionId), eq(trainingSessions.trainerId, trainerId), eq(trainingSessions.status, 'ended')))
  .get();
  ...
  const attendanceByTrainee = new Map(attendanceRows.map((row) => [row.traineeId, row]));
  const scoresByTrainee = examRows.reduce<Record<number, { pretest?: number | null; posttest?: number | null }>>((acc, row) => {
    acc[row.traineeId] ??= {};
    if (row.type === 'pretest') acc[row.traineeId].pretest = row.score;
    if (row.type === 'posttest') acc[row.traineeId].posttest = row.score;
    return acc;
  }, {});
  ...
  const present = participants.filter((participant) => {
    const record = attendanceByTrainee.get(participant.traineeId);
    return record?.status === 'present' || record?.status === 'late';
  }).length;
  const preScores = participants.map((participant) => scoresByTrainee[participant.traineeId]?.pretest).filter((value): value is number => value != null);
  const postScores = participants.map((participant) => scoresByTrainee[participant.traineeId]?.posttest).filter((value): value is number => value != null);
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href="/dashboard/trainer/history" className="text-sm text-primary font-medium hover:underline">Kembali ke riwayat</Link>
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-foreground">Detail Riwayat Training</h1>
        <p className="text-gray-500 dark:text-gray-400">Absensi, nilai, dan durasi sesi training yang sudah diakhiri.</p>
      </div>
        ...
        <div className="p-5 border-b border-border">
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">{item.category || 'General'}{item.location ? ` · ${item.location}` : ''}</p>
              <p className="mt-2 text-sm text-gray-500">Mulai aktual: {formatDate(started)} · Selesai: {formatDate(ended)}</p>
            </div>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-border">
              {participants.length === 0 ? (
                ...
                const attendanceRecord = attendanceByTrainee.get(participant.traineeId);
                const scores = scoresByTrainee[participant.traineeId] ?? {};
                return (
